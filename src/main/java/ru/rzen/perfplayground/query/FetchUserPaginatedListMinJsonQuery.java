...
public class FetchUserPaginatedListMinJsonQuery implements PaginatedJooqQuery {
    ...
    public Page<UserJooqDTO> handle(JooqFilter filter, Pageable pageable) {
        ...
        var ids = getPaginatedResult(
            pageable, query, () -> getCount(condition), UUID.class
        );
        ...
        var users = dsl.select(
            ...
            .leftJoin(SUBDIVISION)
            .on(USERS.SUBDIVISION_ID.eq(SUBDIVISION.ID))
            .where(USERS.ID.in(ids.getContent()))
            .orderBy(order)
